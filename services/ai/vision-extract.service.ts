 ...
 */
async function optimizeImageBuffer(img: Buffer, mimetype: string): Promise<Buffer> {
  ...
  if ((metadata.width && metadata.width > 1024) || (metadata.height && metadata.height > 1024)) {
    image.resize({ width: 1024, height: 1024, fit: 'inside', withoutEnlargement: true });
  }
  // Compress (for JPEG)
 ...
 */
async function extractMenuFromImageWithAI(img: Buffer, options: MenuExtractionOptions = {}, mimetype?: string): Promise<ExtractedMenuItem[]> {
  ...
  try {
    ...
    if (!Array.isArray(items)) throw new Error('AI response is not a JSON array.');
  } catch (err) {
...
export async function extractMenuFromFile(
  ...
  if (!fileType.startsWith('image/') && fileType !== 'application/pdf') {
    errors.push('Unsupported file type. Only images (JPG, PNG) and PDFs are allowed.');
  ...
  if (fileType === 'application/pdf') {
    try {
      ...
      if (imageBuffers.length < 5) {
        warnings.push(`Only ${imageBuffers.length} page(s) extracted from PDF.`);
      }
    } catch (err) {
  ...
  for (const img of imageBuffers) {
    try {
      ...
      const aiItems = await extractMenuFromImageWithAI(optimizedImg, options, fileType);
      if (aiItems.length === 0) {
        warnings.push('No menu items found in one of the images.');
      }
      items.push(...aiItems);
    } catch (err) {
      warnings.push('AI extraction failed for one image: ' + (err as Error).message);
    }
  }
